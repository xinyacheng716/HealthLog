...
import {
  loadDailyMed, saveDailyMed, loadMarkedDates,
  loadAppointments, saveAppointments,
} from '../storage';
import { deleteAppointmentFromCloud } from '../lib/cloudSync';
import { useViewer } from '../context/ViewerContext';
import { fetchOwnerAppointments, fetchOwnerDailyMed, fetchOwnerMedList, fetchOwnerAllMedKeys } from '../lib/viewerData';
import {
...
export default function DailyMedScreen() {
  ...
  useEffect(() => {
    if (!activeDate) { setChecked({}); return; }
  ...
  useEffect(() => {
    if (!isViewerMode || !activeOwner || !activeDate) { setViewerChecked({}); return; }
...
const styles = StyleSheet.create({
  ...
  // Detail header (calendar drilled)
  detailHeader: {
    flexDirection: 'row',
    alignItems: 'center',
